...
def main(args):
    ...
    def make_binary_matrix_dataset(input_prefix, output_prefix, lang, num_workers): # input_prefix为训练/验证/测试集共用的前缀,output_prefix为"train/vaild/test"
        nseq = [0]
        ...
        ds = indexed_dataset.make_builder(dataset_dest_file(args, output_prefix, lang, "bin"), # 根据impl(最终数据集格式),选择创建数据集创建器
                                          impl=args.dataset_impl)  # dataset_dest_file用于获得最终数据数据集的完整文件路径(全部句子的np数组)
        merge_result(
            Binarizer.binarize_matrixs(
                input_file, utils.parse_matrix, lambda t: ds.add_item(t),
                offset=0, end=offsets[1]
            )
        )  # 先定义静态函数binarize(不要求强制传入参数),然后确定binarize中consumer参数,最后进行merge_result
           # 前两步已经完成了对原始数据集的数字化处理,并写入到输出数据集的路径下, 后一步将存储索引为unk_index的单词的计数器同步到replaced计数器,记录该数据集的句子总数和单词总数
    ...
    # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
    def make_all_matrixs(lang):
        if args.trainpref:
            make_binary_matrix_dataset(args.trainpref + "." + args.matrix_suffix, "train." + args.matrix_suffix, lang, num_workers=args.workers)
        if args.validpref:
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(k) if k > 0 else "valid"
                make_binary_matrix_dataset(validpref + "." + args.matrix_suffix, outprefix + "." + args.matrix_suffix, lang, num_workers=args.workers)
        if args.testpref:
            for k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "test{}".format(k) if k > 0 else "test"
                make_binary_matrix_dataset(testpref + "." + args.matrix_suffix, outprefix + "." + args.matrix_suffix, lang, num_workers=args.workers)
    ...
    # <<改写>>, 增加目标文本的后缀,便于添加2种参考译文,shuffle和align;
    if args.matrix_suffix:
        make_all_matrixs(args.source_lang)
...
# <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
def binarize_matrixs(args, filename, parse_matrix, output_prefix, lang, offset, end):
    ds = indexed_dataset.make_builder(dataset_dest_file(args, output_prefix, lang, "bin"),
    ...
    res = Binarizer.binarize_matrixs(filename, parse_matrix, consumer, offset=offset,
                                     end=end)
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
