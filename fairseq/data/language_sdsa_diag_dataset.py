...
def collate(
    ...
    # 使用long,int,short,int8等数据类型影响不大,因为一批matrix参数大小在2M以内
    # 给eos的语法距离序列填充为该批Tree最大语法距离的最大值+1,填充元素会被key_padding_mask处理
    # 注: 特殊符号<pad>,</s>的处理: source <pad><src_str><eos>
    # import time
    # c_time = time.time()
    matrix = None
    if samples[0].get('matrix', None) is not None:
        # 提前打包展平下三角矩阵和源句长,注:s['source']在源句基础上添加了eos,故句长+1
        res_src_len = [(s['matrix'], len(s['source']) - 1) for s in samples]

        max_d = max([max(s['matrix']) for s in samples]) + 1  # 获得该批Tree最大语法距离的最大值+1
        bsz, src_sz = src_tokens.shape  # 获得源句子个数和最大句子长度

        matrix = res_src_len[0][0].new(bsz, src_sz, src_sz).fill_(max_d)  # 创建语法依存矩阵matrix

        for idx, (res, src_len) in enumerate(res_src_len):
            res_matrix = res.new(src_len, src_len).fill_(0)
            # res_matrix = torch.zeros((src_len, src_len), dtype=torch.long)  # 初始化未填充的语法依存矩阵x_dp_i
            # 将res数组转为下三角矩阵,并与转置相加
            xs, ys = torch.tril_indices(src_len, src_len, offset=-1)
            res_matrix[xs, ys] = res
            res_matrix = res_matrix - res_matrix.T  # -=会出错
            # 将每个语法依存矩阵matrix_i放入matrix中
            matrix[idx, (src_sz - src_len - 1): (src_sz - 1), (src_sz - src_len - 1): (src_sz - 1)] = res_matrix
            # 将每个语法依存矩阵matrix_i的对角线置0
            matrix[idx, range(src_sz), range(src_sz)] = 0

        matrix = matrix.index_select(0, sort_order)
    # print('{} sents cost {}s'.format(bsz, time.time()- c_time))
    ...
    batch = {
        ...
        'net_input': {
            ...
            'matrix': matrix,  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        },
...
class LanguageSdsaDiagDataset(FairseqDataset):  # <<改写>>,sdsa模型的模块化
    """
    ...
    def __init__(
        ...
        matrix=None, matrix_sizes=None,  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        ...
        self.tgt = tgt
        self.matrix = matrix  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        self.src_sizes = np.array(src_sizes)  # 将源和目标句子长度列表,转换成np数组
        self.tgt_sizes = np.array(tgt_sizes) if tgt_sizes is not None else None
        self.matrix_sizes = np.array(matrix_sizes) if matrix_sizes is not None else None # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
    ...
    def __getitem__(self, index):
        ...
        if self.matrix is not None:
            example['matrix'] = self.matrix[index]
    ...
    def size(self, index):
        ...
        return (self.src_sizes[index], self.tgt_sizes[index] if self.tgt_sizes is not None else 0,
                self.matrix_sizes[index] if self.matrix_sizes is not None else 0)  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;

    def ordered_indices(self):  # 此处是按源/目标句子长度重新排列,matrix不需要被处理,只需要用这个排列即可
    ...
    def supports_prefetch(self):
        return (
            ...
            and (getattr(self.matrix, 'supports_prefetch', False) or self.matrix is None) # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        )
