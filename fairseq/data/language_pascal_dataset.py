...
def collate(
    ...
    # 特殊符号<pad>,</s>的处理: target <tgt_str><eos>
    matrix = None
    if samples[0].get('matrix', None) is not None:
        s_matrix = [s['matrix'] for s in samples]
        bsz, src_sz = src_tokens.shape  # 获得源句子个数和最大句子长度
        matrix = s_matrix[0].new(bsz, src_sz).fill_(src_sz-1) # 每个句子最后的eos的pascal距离填充为max_len-1

        for i, res in enumerate(s_matrix):  # 取出当前遍历的batch中的每个源/目标句子v,以及在该批次中其为第i句
            matrix[i][:len(res)] = res
...
class LanguagePascalDataset(FairseqDataset):  # <<改写>>,pascal模型的模块化
    """
    ...
    def __getitem__(self, index):
        ...
        matrix_item = self.matrix[index] if self.matrix is not None else None # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        ...
        if self.remove_eos_from_source:
            ...
            if self.src[index][-1] == eos:
                ...
                matrix_item = self.matrix[index][:-1] # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;

        example = {
            'id': index,
            'source': src_item,
            'target': tgt_item,
            'matrix': matrix_item, # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        }
