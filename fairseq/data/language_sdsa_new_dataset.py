...
def collate(
    ...
    if samples[0].get('matrix', None) is not None:
        ...
        matrix = res_src_len[0][0].new(bsz, src_sz, src_sz).fill_(0)  # 创建语法依存矩阵matrix

        # 填充和eos对应值为每个句子的最大语法距离+1;
        for idx, (res, _) in enumerate(res_src_len):
            matrix[idx].fill_(max(res)+1)
...
class LanguageSdsaNewDataset(FairseqDataset):  # <<改写>>,sdsa模型的模块化
    """
