...
def load_langpair_dataset(
    ...
    src, src_dict,
    tgt, tgt_dict,
    matrix_suffix,  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
    ...
    matrix_dataset = None
    if matrix_suffix is not None:
        matrix_prefix = os.path.join(data_path, '{}.{}.{}-{}.'.format(split, matrix_suffix, src, tgt))
        matrix_path = matrix_prefix + src
        if indexed_dataset.dataset_exists(matrix_path, impl=dataset_impl):
            matrix_dataset = data_utils.load_indexed_dataset(matrix_path, None, dataset_impl)
    matrix_dataset_sizes = matrix_dataset.sizes if matrix_dataset is not None else None

    tgt_dataset_sizes = tgt_dataset.sizes if tgt_dataset is not None else None  # 获取目标句子长度列表
    return LanguageSdsaOpenNMTDataset(
        ...
        matrix_dataset, matrix_dataset_sizes,  # <<改写>>, 增加目标文本的后缀,便于添加2种参考译文,shuffle和align;
...
@register_task('translation_sdsa_opennmt')  # <<改写>>,sdsa模型的模块化
class TranslationSdsaOpenNMTTask(FairseqTask):  # <<改写>>,sdsa模型的模块化
    """
    ...
    def add_args(parser):
        ...
        # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
        parser.add_argument("--matrix-suffix", metavar="FP", default=None,
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            ...
            matrix_suffix=self.args.matrix_suffix,  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths):
        return LanguageSdsaOpenNMTDataset(src_tokens, src_lengths, self.source_dictionary)  # <<改写>>, 增加输入矩阵文本的后缀,便于添加sdsa和pascal矩阵;
