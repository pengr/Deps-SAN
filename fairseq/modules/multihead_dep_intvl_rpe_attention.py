...
# <<改写>>,借鉴OpenNMT的rpe模型的模块化
def generate_relative_positions_matrix(matrix, max_dependency_interval):
    """Generate the clipped relative positions matrix
       for a given length and maximum relative positions"""
    distance_mat_clipped = torch.clamp(matrix,
                                       min=-max_dependency_interval,
                                       max=max_dependency_interval)
    # Shift values to be >= 0
    final_mat = distance_mat_clipped + max_dependency_interval
    return final_mat


# <<改写>>,借鉴OpenNMT的rpe模型的模块化
def relative_matmul(x, z, transpose):
    """RK: x: shape(bsz * self.num_heads, tgt_len, self.head_dim);
        z: shape(bsz, src_len, src_len, self.head_dim)
        x_z: shape(bsz * tgt_len, self.num_heads, src_len)
        x_z_t: shape(bsz * self.num_heads, tgt_len, src_len)
       RV: x: shape(bsz * self.num_heads, tgt_len, src_len);
           z: shape(bsz, src_len, src_len, self.head_dim)
           x_z: shape(bsz * tgt_len, self.num_heads, self.head_dim)
           x_z_t: shape(bsz * self.num_heads, tgt_len, self.head_dim)
        """
    bsz_num_heads, tgt_len, _ = x.size()  # Rk: _为head_dim; Rv:_为src_len
    bsz, src_len, src_len, head_dim = z.size()

    if transpose:
        x_z = torch.bmm(x.contiguous().view(bsz*tgt_len, -1, head_dim),
                        z.view(-1, src_len, head_dim).transpose(1, 2))
    else:
        x_z = torch.bmm(x.contiguous().view(bsz * tgt_len, -1, src_len),
                        z.view(-1, src_len, head_dim))
    x_z = x_z.view(bsz_num_heads, tgt_len, -1)
    return x_z


@with_incremental_state
class MultiheadDepIntvlRpeAttention(nn.Module):  # <<改写>>,DepIntvlRpe模型的模块化
    ...
    def __init__(
        ...
        max_dependency_interval=0,  # <<改写>>,DepIntvlRpe模型的模块化
        no_key_dep_intvl_rpe=False,  # <<改写>>,DepIntvlRpe模型的模块化
        no_value_dep_intvl_rpe=False,  # <<改写>>,DepIntvlRpe模型的模块化
        ...
        # <<改写>>,借鉴OpenNMT的rpe模型的模块化
        self.max_dependency_interval = max_dependency_interval
        # <<改写>>,用于DepIntvlRpe模型的RK,RV消融
        self.no_key_dep_intvl_rpe = no_key_dep_intvl_rpe
        self.no_value_dep_intvl_rpe = no_value_dep_intvl_rpe

        if max_dependency_interval > 0:
            vocab_size = max_dependency_interval * 2 + 1
    ...
    def forward(
        ...
        sdsa: Optional[Tensor],  # <<改写>>, 添加sdsa和pascal矩阵;
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadDepIntvlRpeAttention._append_prev_key_padding_mask(  # <<改写>>,DepIntvlRpe模型的模块化
        ...
        # <<改写>>,借鉴OpenNMT的DepIntvlRPE模型的模块化,仅适用于编码端sefl-attn,用于DepIntvlRpe模型的RK,RV消融
        if self.max_dependency_interval > 0 and self.self_attention:
            # 1 or bsz x src_len x src_len
            relative_positions_matrix = generate_relative_positions_matrix(
                sdsa, self.max_dependency_interval)
            # 1 or bsz x src_len x src_len x head_dim
            relations_keys = self.relative_positions_embeddings(
                relative_positions_matrix) if not self.no_key_dep_intvl_rpe else None
            # 1 or bsz x src_len x src_len x head_dim
            relations_values = self.relative_positions_embeddings(
                relative_positions_matrix) if not self.no_value_dep_intvl_rpe else None
        ...
        attn_weights = MultiheadDepIntvlRpeAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz) # <<改写>>,DepIntvlRpe模型的模块化 # 此函数直接返回attn_weights

        assert list(attn_weights.size()) == [bsz * self.num_heads, tgt_len, src_len]  # 确认attn_weights的维度无误

        # <<改写>>,借鉴OpenNMT的DepIntvlRPE模型的模块化,仅适用于编码端sefl-attn,用于DepIntvlRpe模型的RK,RV消融
        # 不担心relative potision embedding的填充,是因为attn_weights由key_padding_mask进行填充-inf
        if self.max_dependency_interval > 0 and self.self_attention and relations_keys is not None:
            attn_weights = attn_weights + relative_matmul(q, relations_keys, True)
        ...
        # <<改写>>,借鉴OpenNMT的DepIntvlRPE模型的模块化,仅适用于编码端sefl-attn,用于DepIntvlRpe模型的RK,RV消融
        # 不担心relative potision embedding的填充,是因为attn_weights经过softmax,填充值为0
        if self.max_dependency_interval > 0 and self.self_attention and relations_values is not None:
            attn = attn + relative_matmul(attn_probs, relations_values, False)
