...
class MultiheadPascalAttention(nn.Module):  # <<改写>>,pascal模型的模块化
    ...
    def __init__(
        ...
        embed_dim,
        num_heads,
        num_pascal_heads,     # <<改写>>,pascal模型的模块化
        kdim=None,
        vdim=None,
        dropout=0.0,
        parent_ignoring=0.0,  # <<改写>>,pascal模型的模块化
        weight_fn='normal',  # <<改写>>,pascal模型的模块化
        weight_sigma=2,  # <<改写>>,pascal模型的模块化
        bias=True,
        add_bias_kv=False,
        add_zero_attn=False,
        ...
        self.num_pascal_heads = num_pascal_heads  # <<改写>>, 加入pascal的注意力头数
        self.dropout = dropout      # 获取self-attn的dropout率,由args.attention_dropout给定
        self.parent_ignoring = parent_ignoring  # <<改写>>,设置pascal矩阵的parent ignoring概率
    ...
    # <<改写>>, xs - parents.unsqueeze(-1)表示j-pt所形成的矩阵,维度为(batch,src_len,src_len)
    ...
    def forward(
        ...
        pascal: Optional[Tensor],   # <<改写>>, 添加sdsa和pascal矩阵;
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadPascalAttention._append_prev_key_padding_mask(  # <<改写>>,pascal模型的模块化
        ...
        attn_weights = MultiheadPascalAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz) # <<改写>>,pascal模型的模块化 # 此函数直接返回attn_weights

        assert list(attn_weights.size()) == [bsz * self.num_heads, tgt_len, src_len]  # 确认attn_weights的维度无误

        # ------------- <<改写>>,pascal_weights计算和parent ignoring,选择注意力头对scores乘以sdsa_weights -----------
        # # 对一个批次内的句子长度不等引起的填充问题,由key_padding_mask处理
        rng = torch.tensor(range(pascal.size(1))).type_as(pascal)
        pascal_weights = self.weight_fn(rng, pascal, self.weight_sigma).repeat(1, self.num_pascal_heads, 1).view(-1, pascal.size(1),pascal.size(1))

        # <<改写>>, 随机以parent_ignoring概率将pascal矩阵的某一行全置为1
        ...
        # <<改写>>, 根据pascal注意力头数, 对attn_weights的后num_pascal_heads * batch_size矩阵使用高斯分布
