...
class MultiheadSdsaPadAttention(nn.Module):  # <<改写>>,sdsa模型的模块化
    ...
    def __init__(
        ...
        embed_dim,
        num_heads,
        num_sdsa_heads,  # <<改写>>,sdsa模型的模块化
        kdim=None,
        vdim=None,
        dropout=0.0,
        prob_q=0.0,  # <<改写>>,sdsa模型的模块化
        win_k=0.0,
        weight_fn='normal',
        weight_sigma=2,
        weight_usage='mult',  # <<改写>>,sdsa模型的模块化
        bias=True,
        add_bias_kv=False,
        add_zero_attn=False,
        ...
        self.num_sdsa_heads = num_sdsa_heads  # <<改写>>, 加入sdsa的注意力头数
        self.dropout = dropout      # 获取self-attn的dropout率,由args.attention_dropout给定
        self.prob_q = prob_q  # <<改写>>,设置语法距离矩阵的忽视或稀疏概率
        # self.binomial = torch.distributions.binomial.Binomial(probs=1 - prob_q)  # <<改写>>,依赖语法距离矩阵的二项分布函数
        self.win_k = win_k  # <<改写>>,设置语法距离矩阵的语法距离阈值
        ...
        self.weight_sigma = weight_sigma  # <<改写>>,编码端标准差
        self.weight_usage = weight_usage  # <<改写>>,如何使用高斯分布的方法
    ...
    # <<改写>>, 用sdsa直接替换掉xs - parents.unsqueeze(-1),两者均表示j-pt所形成的矩阵,维度为(batch,src_len,src_len)
    def normpdf(self, sdsa, sd):
        var = sd**2
        denom = (2*math.pi*var)**.5
        return torch.exp(-(sdsa)**2/(2*var))/denom

    # <<改写>>, 用sdsa直接替换掉xs - parents.unsqueeze(-1),两者均表示j-pt所形成的矩阵,维度为(batch,src_len,src_len)
    def unipdf(self, sdsa, w):
        return torch.lt(torch.abs(sdsa), w+1).type(torch.cuda.FloatTensor)
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadSdsaPadAttention._append_prev_key_padding_mask(  # <<改写>>,sdsa模型的模块化
        ...
        attn_weights = MultiheadSdsaPadAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz) # <<改写>>,sdsa模型的模块化 # 此函数直接返回attn_weights

        assert list(attn_weights.size()) == [bsz * self.num_heads, tgt_len, src_len]  # 确认attn_weights的维度无误

        # ------------- sdsa_weights计算和知识稀疏,选择注意力头对scores乘以sdsa_weights -----------
        # 对一个批次内的句子长度不等引起的填充问题,由key_padding_mask处理
        # <<改写>>, 随机或阈值为k来显式稀疏构建语法距离矩阵的矩阵, 即设置该位置为该批Tree最大语法距离的最大值+1
        if self.training and self.prob_q > 0.:
            rand_mask = F.dropout(torch.ones_like(sdsa), p=self.prob_q).to(torch.bool) # 创建一个以prob_q概率变0,与sdsa同形状的0-1矩阵
            sdsa = sdsa.masked_fill(~rand_mask, torch.max(sdsa))
            # samples = self.binomial.sample(sdsa.size()).type(torch.cuda.FloatTensor)
            # sdsa = sdsa.masked_fill(~samples.bool(), torch.max(sdsa) + 1)
        elif self.training and self.win_k > 0.:
            k_mask = torch.gt(sdsa, self.win_k)
            sdsa = sdsa.masked_fill(k_mask, torch.max(sdsa))

        # <<改写>>,由weight_fn生成语法指导矩阵的概率分布bsz, src_len, src_len,并扩维->bsz*num_sdsa_heads,src_len, src_len
        # 由于enc-side SAN,故q,k,v均一样,即src_len=tgt_len=sdsa.size(1)
        # 必须先repeat第2维后view(),因为attn_weights的第一维bsz*num_sdsa_heads是按照sent1*num_sdsa_heads,...sent(bsz)*num_sdsa_heads排列的
        sdsa_weights = self.weight_fn(sdsa, self.weight_sigma).repeat(1, self.num_sdsa_heads, 1).view(-1, sdsa.size(1), sdsa.size(1))
        sdsa_weights = sdsa_weights.masked_fill(key_padding_mask.view(-1, 1).repeat(1, src_len).view(bsz, src_len, src_len).repeat_interleave(
            self.num_sdsa_heads, dim=0).to(torch.bool), 1.)

        # <<改写>>, 根据sdsa注意力头数, 对attn_weights的后num_dsa_heads * batch_size矩阵使用高斯分布
        if self.weight_usage == "mult":
            attn_weights[-self.num_sdsa_heads * bsz:] *= sdsa_weights
        elif self.weight_usage == "add":
            attn_weights[-self.num_sdsa_heads * bsz:] += sdsa_weights
