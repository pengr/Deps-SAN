...
def relative_matmul(x, z, transpose):
    """RK: x: shape(bsz * self.num_heads, tgt_len, self.head_dim);
        z: shape(src_len, src_len, self.head_dim)
        x_z: shape(tgt_len, bsz * self.num_heads, src_len)
        x_z_t: shape(bsz * self.num_heads, tgt_len, src_len)
       RV: x: shape(bsz * self.num_heads, tgt_len, src_len);
           z: shape(src_len, src_len, self.head_dim)
           x_z: shape(tgt_len, bsz * self.num_heads, self.head_dim)
           x_z_t: shape(bsz * self.num_heads, tgt_len, self.head_dim)
        """
    if transpose:
        x_z = torch.bmm(x.transpose(0, 1), z.transpose(1, 2))
    else:
        x_z = torch.bmm(x.transpose(0, 1), z)
    x_z_t = x_z.transpose(0, 1)
    return x_z_t


@with_incremental_state
class MultiheadRpeAttention(nn.Module): # <<改写>>,rpe模型的模块化
    ...
    def __init__(
        ...
        max_relative_positions=0,  # <<改写>>,rpe模型的模块化
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadRpeAttention._append_prev_key_padding_mask(  # <<改写>>,rpe模型的模块化
        ...
        # <<改写>>,借鉴OpenNMT的rpe模型的模块化
        if self.max_relative_positions > 0 and self.self_attention:
            src_len = k.size(1)
            # 1 or src_len x src_len
            relative_positions_matrix = generate_relative_positions_matrix(
                src_len, self.max_relative_positions,
                cache=True if saved_state is not None else False)
            #  1 or src_len x src_len x dim_per_head
        ...
        attn_weights = MultiheadRpeAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz) # <<改写>>,rpe模型的模块化 # 此函数直接返回attn_weights
