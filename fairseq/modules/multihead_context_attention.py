...
class MultiheadContextAttention(nn.Module):  # <<改写>>,Context模型的模块化
    ...
    def __init__(
        ...
        embed_dim,
        num_heads,
        num_context_heads,     # <<改写>>,context模型的模块化
        num_layers,     # <<改写>>,context模型的模块化
        kdim=None,
        vdim=None,
        dropout=0.0,
        context_type='deep_global',  # <<改写>>,context模型的模块化
        bias=True,
        add_bias_kv=False,
        add_zero_attn=False,
        ...
        self.num_context_heads = num_context_heads  # <<改写>>, 加入context的注意力头数
        self.num_layers = num_layers  # <<改写>>, 加入context的层号
        ...
        # <<改写>>, self化context-aware self-attn的四种窗口类型
        self.context_type = context_type

        # <<改写>>, 对应V(H,Q)和V(H,K);V(C,Q)和V(C,K);
        self.vq_H = nn.Linear(self.head_dim, 1, bias=bias)
        self.vk_H = nn.Linear(self.head_dim, 1, bias=bias)
        self.vq_C = nn.Linear(self.head_dim, 1, bias=bias)
        self.vk_C = nn.Linear(self.head_dim, 1, bias=bias)

        # 对应U(Q)和U(K);
        if self.context_type == "global":
            self.uq = nn.Linear(self.head_dim, self.head_dim, bias=bias)
            self.uk = nn.Linear(self.head_dim, self.head_dim, bias=bias)
        # 注意num_layers从0开始,即使0对应第一层; deep context第1层无uq和uk
        elif self.context_type == "deep":
            self.uq = nn.Linear(num_layers*self.head_dim, self.head_dim, bias=bias) if num_layers > 0 else None
            self.uk = nn.Linear(num_layers*self.head_dim, self.head_dim, bias=bias) if num_layers > 0 else None
        # 注意num_layers从0开始,即使0对应第一层; deep-global context第1层就有uq和uk
        elif self.context_type == "deep_global":
            self.uq = nn.Linear((num_layers+1)*self.head_dim, self.head_dim, bias=bias)
            self.uk = nn.Linear((num_layers+1)*self.head_dim, self.head_dim, bias=bias)
        # elif self.context_type == "deep_global_deep":
        #     self.uq_dg = nn.Linear(num_layers * self.head_dim, self.head_dim, bias=bias) if num_layers > 0 else None
        #     self.uk_dg = nn.Linear(num_layers * self.head_dim, self.head_dim, bias=bias) if num_layers > 0 else None
        #     self.uq_dgd = nn.Linear((num_layers + 1) * self.head_dim, self.head_dim, bias=bias)
        #     self.uk_dgd = nn.Linear((num_layers + 1) * self.head_dim, self.head_dim, bias=bias)

        # <<改写>>,按照context type-初始化V(H,Q)和V(H,K);V(C,Q)和V(C,K);U(Q)和U(K);
        self.reset_context_parameters(context_type,num_layers)
        ...
        # <<改写>>,训练时,编码器的self-attn均由pytorch底层实现,阻止了对self-attn的修改;
    ...
    # <<改写>>,按照context type-初始化V(H,Q)和V(H,K);V(C,Q)和V(C,K);U(Q)和U(K);
    def reset_context_parameters(self, context_type, num_layers):
        nn.init.xavier_uniform_(self.vq_H.weight)
        nn.init.xavier_uniform_(self.vk_H.weight)
        nn.init.xavier_uniform_(self.vq_C.weight)
        nn.init.xavier_uniform_(self.vk_C.weight)
        if context_type in ["global","deep_global"]:
            nn.init.xavier_uniform_(self.uq.weight)
            nn.init.xavier_uniform_(self.uk.weight)
        elif context_type == "deep":
            nn.init.xavier_uniform_(self.uq.weight) if num_layers > 0 else None
            nn.init.xavier_uniform_(self.uk.weight) if num_layers > 0 else None
        # elif self.context_type == "deep_global_deep":
        #     nn.init.xavier_uniform_(self.uq_dg.weight)
        #     nn.init.xavier_uniform_(self.uk_dg.weight)
        #     nn.init.xavier_uniform_(self.uq_dgd.weight)
        #     nn.init.xavier_uniform_(self.uk_dgd.weight)
    ...
    def forward(
        ...
        context: Optional[Tensor],   # <<改写>>,加入编码器的上下文向量C
        ...
        # deep context: 第一层C为None
        if context is not None:
            if self.context_type == "global":
                context = (
                        context.contiguous()
                            .view(1, bsz * self.num_heads, self.head_dim)
                            .transpose(0, 1)  # (bsz*num_heads, 1, head_dim)
                    )
            elif self.context_type == "deep":
                context = (
                    context.contiguous()
                        .view(tgt_len, bsz * self.num_heads, -1)
                        .transpose(0, 1)   # (bsz*num_heads, tgt_len, (l-1)*head_dim)
                )
            elif self.context_type == "deep_global":
                context = (
                    context.contiguous()
                        .view(1, bsz * self.num_heads, -1)
                        .transpose(0, 1)   # (bsz*num_heads, １, (l-1)*head_dim)
                )

            lambda_q = torch.sigmoid(self.vq_H(q) + self.vq_C(self.uq(context)))
            lambda_k = torch.sigmoid(self.vk_H(k) + self.vk_C(self.uk(context)))

            q = (1. - lambda_q) * q + lambda_q * self.uq(context)  # (batch, tgt_len, dim)
            k = (1. - lambda_k) * k + lambda_k * self.uk(context)  # (batch, src_len, dim)
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadContextAttention._append_prev_key_padding_mask(  # <<改写>>,context模型的模块化
        ...
        attn_weights = MultiheadContextAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz) # <<改写>>,context模型的模块化 # 此函数直接返回attn_weights

        assert list(attn_weights.size()) == [bsz * self.num_heads, tgt_len, src_len]  # 确认attn_weights的维度无误

        # ----- <<改写>>,local_weights计算,选择注意力头对attn_weigght乘以local_weights ----
        # 对一个批次内的句子长度不等引起的填充问题,由key_padding_mask处理
        # 预测对齐位置: pi=UpTtanh(WpQi); Pi=I·sigmoid(pi)
        # p_t = self.up_align(torch.tanh(self.wp_align(q)))  # (bsz*num_heads,tgt_len,1)
        # # 注: 对齐位置pi未用作索引,故不必担心超出句子索引而用句长-1
        # src_lengths_repeat = src_lengths.repeat_interleave(self.num_heads).view(-1, 1, 1)
        # p_t = src_lengths_repeat * torch.sigmoid(p_t)  # (bsz*num_heads,tgt_len,1)
        #
        # # 按照window type来预测窗口大小D
        # win_size = None
        # if self.win_type == "fixed":
        #     win_size = 10
        # elif self.win_type == "layer_spec":  # 对key(bsz*num_heads,src_len,head_dim)的第2维取平均(即每个时间步)
        #     win_size = self.ud_window(torch.tanh(self.wd_window(k.mean(dim=1, keepdim=True))))  # (bsz * num_heads,1,1)
        #     win_size = src_lengths_repeat * torch.sigmoid(win_size)  # (bsz * num_heads,1,1)
        # elif  self.win_type == "query_spec":
        #     win_size = self.ud_window(torch.tanh(self.wp_align(q)))  # (bsz * num_heads,tgt_len,1)
        #     win_size = src_lengths_repeat * torch.sigmoid(win_size)  # (bsz * num_heads,tgt_len,1)
        #
        # # <<改写>>,由weight_fn完成本地距离矩阵的概率分布,(bsz * num_heads,tgt_len,tgt_len)
        # local_weights = self.weight_fn((torch.tensor(range(p_t.size(1))).type_as(p_t) - p_t), win_size / 2)
        # # <<改写>>, 根据localness注意力头数, 对attn_weights的后num_local_heads * batch_size矩阵使用高斯分布,将高斯分布权重值G作为bias相加
        # attn_weights[-self.num_local_heads * bsz:] += local_weights
