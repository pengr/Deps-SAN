...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadDepIntvlRpeAttention  # <<改写>>,DepIntvlRpe模型的模块化
from torch import Tensor


class TransformerDepIntvlRpeEncoderLayer(nn.Module):   # <<改写>>,DepIntvlRpe模型的模块化
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def build_self_attention(self, embed_dim, args):# 根据编码器词嵌入维数,注意力头数,注意力dropout率,以及是否为self-attn的标记返回MultiheadAttention类
        return MultiheadDepIntvlRpeAttention(   # <<改写>>,DepIntvlRpe模型的模块化
            embed_dim,
            args.encoder_attention_heads,
            dropout=args.attention_dropout,
            self_attention=True,
            max_dependency_interval=args.max_dependency_interval,  # <<改写>>,DepIntvlRpe模型的模块化
            no_key_dep_intvl_rpe=args.no_key_dep_intvl_rpe,        # <<改写>>,DepIntvlRpe模型的模块化
            no_value_dep_intvl_rpe=args.no_value_dep_intvl_rpe,    # <<改写>>,DepIntvlRpe模型的模块化
