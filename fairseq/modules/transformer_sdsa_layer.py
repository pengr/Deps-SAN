...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadSdsaAttention  # <<改写>>,sdsa模型的模块化
from torch import Tensor


class TransformerSdsaEncoderLayer(nn.Module):  # <<改写>>,sdsa模型的模块化
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, num_sdsa_heads):  # <<改写>>,sdsa模型的模块化
        super().__init__()
        self.embed_dim = args.encoder_embed_dim   # 编码器词嵌入维数
        self.num_sdsa_heads = num_sdsa_heads  # <<改写>>,sdsa模型的模块化
        self.self_attn = self.build_self_attention(self.embed_dim, args, self.num_sdsa_heads)  # <<改写>>,sdsa模型的模块化 # 返回所定义好的MultiheadAttention类(其中由enable_torch_version来决定是否使用torch底层的multi_headed attn实现)
    ...
    def build_self_attention(self, embed_dim, args, num_sdsa_heads):  # <<改写>>,sdsa模型的模块化  # 根据编码器词嵌入维数,注意力头数,注意力dropout率,以及是否为self-attn的标记返回MultiheadAttention类
        return MultiheadSdsaAttention(  # <<改写>>,sdsa模型的模块化
            embed_dim,
            args.encoder_attention_heads,
            dropout=args.attention_dropout,
            self_attention=True,
            num_sdsa_heads=num_sdsa_heads,
            prob_q=args.prob_q,
            win_k=args.win_k,
            weight_fn=args.weight_fn,
            weight_sigma=args.weight_sigma,
            weight_usage=args.weight_usage,
    ...
    def forward(self, x, encoder_padding_mask, matrix, attn_mask: Optional[Tensor] = None):  # <<改写>>, 添加sdsa和pascal矩阵;
        """
        Args:
        ...
        x, attn_weights = self.self_attn( # 由编码器层输入x作为query,key,value,再将源端填充掩码矩阵,以及attn_mask(在计算t_tgt的嵌入时用于掩码t_src)
            query=x,
            key=x,
            value=x,
            sdsa=matrix,  # <<改写>>, 添加sdsa和pascal矩阵;
        ...
        return x, attn_weights  # 返回当前编码器层的输出layer_norm(dropout(max(0, x'W1+b1)W2+b2) + x')
