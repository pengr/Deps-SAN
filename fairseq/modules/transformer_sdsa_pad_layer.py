...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadSdsaPadAttention  # <<改写>>,sdsa模型的模块化
from torch import Tensor


class TransformerSdsaPadEncoderLayer(nn.Module):  # <<改写>>,sdsa模型的模块化
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def build_self_attention(self, embed_dim, args, num_sdsa_heads):  # <<改写>>,sdsa模型的模块化  # 根据编码器词嵌入维数,注意力头数,注意力dropout率,以及是否为self-attn的标记返回MultiheadAttention类
        return MultiheadSdsaPadAttention(  # <<改写>>,sdsa模型的模块化
            embed_dim,
