...
class MultiheadLocalnessAttention(nn.Module):  # <<改写>>,localness模型的模块化
    ...
    def __init__(
        ...
        embed_dim,
        num_heads,
        num_local_heads,     # <<改写>>,localness模型的模块化
        kdim=None,
        vdim=None,
        dropout=0.0,
        weight_fn='normal',   # <<改写>>,localness模型的模块化
        win_type='query_spec',  # <<改写>>,localness模型的模块化
        bias=True,
        add_bias_kv=False,
        add_zero_attn=False,
        ...
        self.num_local_heads = num_local_heads  # <<改写>>, 加入localness的注意力头数
        ...
        # <<改写>>, self化窗口大小和窗口类型,以及当前所在端是否使用localness
        self.win_type = win_type
        # <<改写>>,用于预测对齐位置pt的Wp和Up
        self.wp_align = nn.Linear(self.head_dim, self.head_dim, bias=bias)
        self.up_align = nn.Linear(self.head_dim, 1, bias=bias)
        # <<改写>>,按照window type-预测窗口大小D的wd,ud("layer_spec")或Wp,ud("query_spec")
        if win_type == "layer_spec":
            self.wd_window = nn.Linear(self.kdim // num_heads, self.head_dim, bias=bias)
            self.ud_window = nn.Linear(self.head_dim, 1, bias=bias)
        elif win_type == "query_spec":
            self.ud_window = nn.Linear(self.head_dim, 1, bias=bias)

        # <<改写>>,按照window type-初始化预测窗口大小D的wd,ud("layer_spec")或Wp,ud("query_spec")
        self.reset_localness_parameters(win_type)
    ...
    # <<改写>>,按照window type-初始化预测窗口大小D的wd,ud("layer_spec")或Wp,ud("query_spec")
    def reset_localness_parameters(self, win_type):
        nn.init.xavier_uniform_(self.wp_align.weight)
        nn.init.xavier_uniform_(self.up_align.weight)
        if win_type == "layer_spec":
            nn.init.xavier_uniform_(self.wd_window.weight)
            nn.init.xavier_uniform_(self.ud_window.weight)
        elif win_type == "query_spec":
            nn.init.xavier_uniform_(self.ud_window.weight)
    ...
    # <<改写>>, 用local直接替换掉xs - parents.unsqueeze(-1),两者均表示j-pt所形成的矩阵,维度为(batch,src_len,src_len)
    def normpdf(self, local, sd):
        var = sd ** 2
        return -(local) ** 2 / (2 * var)

    # <<改写>>, 用dsa直接替换掉xs - parents.unsqueeze(-1),两者均表示j-pt所形成的矩阵,维度为(batch,src_len,src_len)
    def unipdf(self, local, w):
        return torch.lt(torch.abs(local), w + 1).type(torch.cuda.FloatTensor)
    ...
    def forward(
        ...
        src_lengths: Optional[Tensor],  # <<改写>>, 导入当前批次源句子长度
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadLocalnessAttention._append_prev_key_padding_mask(  # <<改写>>,localness模型的模块化
        ...
        attn_weights = MultiheadLocalnessAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz) # <<改写>>,localness模型的模块化 # 此函数直接返回attn_weights
