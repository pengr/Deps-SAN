...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadRpeAttention   # <<改写>>,rpe模型的模块化
from torch import Tensor


class TransformerRpeEncoderLayer(nn.Module):   # <<改写>>,rpe模型的模块化
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def build_self_attention(self, embed_dim, args):# 根据编码器词嵌入维数,注意力头数,注意力dropout率,以及是否为self-attn的标记返回MultiheadAttention类
        return MultiheadRpeAttention( # <<改写>>,rpe模型的模块化
            embed_dim,
            args.encoder_attention_heads,
            dropout=args.attention_dropout,
            self_attention=True,
            max_relative_positions=args.max_relative_positions, # <<改写>>,rpe模型的模块化
...
class TransformerRpeDecoderLayer(nn.Module): # <<改写>>,rpe模型的模块化
    """Decoder layer block.
