...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadLocalnessAttention  # <<改写>>,localness模型的模块化
from torch import Tensor


class TransformerLocalnessEncoderLayer(nn.Module):  # <<改写>>,localness模型的模块化
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, num_local_heads):  # <<改写>>,localness模型的模块化
        super().__init__()
        self.embed_dim = args.encoder_embed_dim   # 编码器词嵌入维数
        self.num_local_heads = num_local_heads  # <<改写>>,localness模型的模块化
        self.self_attn = self.build_self_attention(self.embed_dim, args, self.num_local_heads)  # <<改写>>,localness模型的模块化 # 返回所定义好的MultiheadAttention类(其中由enable_torch_version来决定是否使用torch底层的multi_headed attn实现)
    ...
    def build_self_attention(self, embed_dim, args, num_local_heads):  # <<改写>>,localness模型的模块化  # 根据编码器词嵌入维数,注意力头数,注意力dropout率,以及是否为self-attn的标记返回MultiheadAttention类
        return MultiheadLocalnessAttention(  # <<改写>>,localness模型的模块化
            embed_dim,
    ...
    def forward(self, x, encoder_padding_mask, src_lengths, attn_mask: Optional[Tensor] = None):  # <<改写>>, 导入当前批次源句子长度
        """
        Args:
        ...
        x, _ = self.self_attn( # 由编码器层输入x作为query,key,value,再将源端填充掩码矩阵,以及attn_mask(在计算t_tgt的嵌入时用于掩码t_src)
            ...
            key=x,
            value=x,
            src_lengths=src_lengths,  # <<改写>>, 导入当前批次源句子长度
