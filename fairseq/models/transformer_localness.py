...
from fairseq.models import (
    FairseqEncoder,   # localness模型的编码器的父类
    FairseqEncoderDecoderModel,
    FairseqIncrementalDecoder,
    register_model,
    register_model_architecture,
)
from fairseq.models.transformer import TransformerDecoder  # localness模型的解码器
...
from fairseq.modules import (
    ...
    TransformerEncoderLayer,   # localness模型的san编码层
    TransformerLocalnessEncoderLayer,   # <<改写>>,localness模型的sdsa编码层
)
...
@register_model("transformer_localness")  # <<改写>>,localness模型的模块化
class TransformerLocalnessModel(FairseqEncoderDecoderModel):  # <<改写>>,localness模型的模块化
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--enc-local-heads', type=int, nargs='+',
                            help='list of localness self-attention heads per layer')
        # <<改写>>,设置Pascal矩阵的的概率分布函数,默认为"normal"
        ...
        # <<改写>>,设置本地距离矩阵的三种窗口类型
        parser.add_argument('--win-type', type=str, metavar='STR',
                            choices=["fixed", "layer_spec", "query_spec"],
                            help="Which window types to use for local distance matrix")
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # 通过args,源字典以及源端词嵌入建立编码器,初始化TransformerEncoder类
        return TransformerLocalnessEncoder(args, src_dict, embed_tokens)  # <<改写>>,Localness模型的模块化 # 返回一个与原始论文一致的Transformer Encoder,每一层TransformerEncoderLayer(Multi-headed attn->dropout->添加残差-> layernorm)
...
class TransformerLocalnessEncoder(FairseqEncoder):  # <<改写>>,localness模型的模块化
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        # <<改写>>,确认localness的注意力头和层数
        if args.enc_local_heads is None:
            self.enc_local_heads = [0] * args.encoder_layers
        else:
            self.enc_local_heads = args.enc_local_heads
        assert len(self.enc_local_heads) == args.encoder_layers
        ...
        self.layers.extend(
            [self.build_localness_encoder_layer(args, h) if h != 0 \
                 else self.build_encoder_layer(args) for h in self.enc_local_heads]
        )   # <<改写>>,localness模型的模块化 # 根据args中编码器层数,由args构建每一层编码器层(与原始论文一致,Multi-headed attn->dropout->添加残差-> layernorm)
    ...
    def build_localness_encoder_layer(self, args, h):  # <<改写>>,localness模型的模块化# 由args初始化一个TransformerEncoderLayer类
        return TransformerLocalnessEncoderLayer(args, h)  # <<改写>>,localness模型的模块化  # 返回一个与原始论文一致的Transformer编码器层,Multi-headed attn->dropout->添加残差-> layernorm
    ...
    def forward(
        ...
        # encoder layers
        for i, h in enumerate(self.enc_local_heads): # <<改写>>, 由enc_local_heads遍历每一个编码器层,并添加sdsa和pascal矩阵;
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):  # 若非训练阶段,或者无layer_dropout(0),即均匀随机初始化的dropout值肯定>0:
                ...
                if h != 0:
                    x = self.layers[i](x, encoder_padding_mask, src_lengths)
                else:
...
@register_model_architecture("transformer_localness", "transformer_localness")  # <<改写>>,localness模型的模块化
def base_architecture(args):
    ...
    # <<改写>>,localness模型的模块化
    args.enc_local_heads = getattr(args, "enc_local_heads", None)
    args.weight_fn = getattr(args, "weight_fn", "normal")
    args.win_type = getattr(args, "win_type", "query_spec")
...
@register_model_architecture("transformer_localness", "transformer_localness_iwslt_de_en")  # <<改写>>,localness模型的模块化
def transformer_localness_iwslt_de_en(args):  # <<改写>>,localness模型的模块化
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_localness", "transformer_localness_wmt_en_de")  # <<改写>>,localness模型的模块化
def transformer_localness_wmt_en_de(args):  # 通过-arch选项选择合适的transformer参数  # <<改写>>,localness模型的模块化
    base_architecture(args)  # 针对args提供进来的参数,已存在的不变,未设置的则按base_architecture设置


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_localness", "transformer_localness_vaswani_wmt_en_de_big")  # <<改写>>,localness模型的模块化
def transformer_localness_vaswani_wmt_en_de_big(args):  # <<改写>>,pascal模型的模块化
...
@register_model_architecture("transformer_localness", "transformer_localness_vaswani_wmt_en_fr_big")  # <<改写>>,localness模型的模块化
def transformer_localness_vaswani_wmt_en_fr_big(args):  # <<改写>>,localness模型的模块化
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_localness_vaswani_wmt_en_de_big(args)  # <<改写>>,localness模型的模块化


@register_model_architecture("transformer_localness", "transformer_localness_wmt_en_de_big")  # <<改写>>,localness模型的模块化
def transformer_localness_wmt_en_de_big(args):  # <<改写>>,localness模型的模块化
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_localness_vaswani_wmt_en_de_big(args)  # <<改写>>,localness模型的模块化


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_localness", "transformer_localness_wmt_en_de_big_t2t")  # <<改写>>,localness模型的模块化
def transformer_localness_wmt_en_de_big_t2t(args):  # <<改写>>,localness模型的模块化
