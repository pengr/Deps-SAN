...
from fairseq.models import (
    FairseqEncoder,   # sdsa模型的编码器的父类
    FairseqEncoderDecoderModel,
    FairseqIncrementalDecoder,
    register_model,
    register_model_architecture,
)
from fairseq.models.transformer import TransformerDecoder  # sdsa模型的解码器
...
from fairseq.modules import (
    ...
    TransformerEncoderLayer,   # sdsa模型的san编码层
    TransformerSdsaEncoderLayer,   # <<改写>>,sdsa模型的sdsa编码层
)
...
@register_model("transformer_sdsa")  # <<改写>>,sdsa模型的模块化
class TransformerSdsaModel(FairseqEncoderDecoderModel):  # <<改写>>,sdsa模型的模块化
    """
    ...
    def add_args(parser):
        ...
        # <<改写>>,设置TransformerSdsaEncoderLayer的层数和注意力头数,列表类型(默认:[8 8 8 0 0 0])
        parser.add_argument('--enc-sdsa-heads', type=int, nargs='+',
                            help='Number of heads and layers for enc-side syntax-directed self-attention')
        # <<改写>>,设置语法指导矩阵的随机忽视和稀疏概率,默认为0.
        parser.add_argument('--prob-q', type=float, metavar='D',
                            help="The sparse and ignoring of probability for syntax-directed matrix")
        # <<改写>>,设置语法指导矩阵的语法距离阈值k
        parser.add_argument('--win-k', type=float, metavar='D',
                            help="The​​ threshold of syntax distance for syntax-directed matrix")
        # <<改写>>,设置语法距离矩阵的的概率分布函数
        parser.add_argument('--weight-fn', type=str, metavar='STR', choices=["normal", "uniform"],
                            help="Weight function used for syntax-directed matrix")
        # <<改写>>,设置语法指导矩阵的的编码端和解码端的标准差, 借鉴n_gram=4, σ=n_gram/2
        parser.add_argument('--weight-sigma', type=float, metavar='D',
                            help="Standard deviation of weight distribution of syntax-directed self-attention")
        # <<改写>>,设置如何使用高斯分布的方法
        parser.add_argument('--weight-usage', type=str, metavar='STR', choices=["mult", "add"],
                            help="Usage of weight distribution for syntax-directed matrix")
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # 通过args,源字典以及源端词嵌入建立编码器,初始化TransformerEncoder类
        return TransformerSdsaEncoder(args, src_dict, embed_tokens)  # <<改写>>,Sdsa模型的模块化 # 返回一个与原始论文一致的Transformer Encoder,每一层TransformerEncoderLayer(Multi-headed attn->dropout->添加残差-> layernorm)
    ...
    def forward(
        ...
        src_lengths,
        prev_output_tokens,
        matrix: Optional[Tensor] = None,   # <<改写>>, 添加sdsa和pascal矩阵;
        ...
        encoder_out = self.encoder(
            ...
            matrix=matrix,    # <<改写>>, 添加sdsa和pascal矩阵;
            cls_input=cls_input,
...
class TransformerSdsaEncoder(FairseqEncoder):  # <<改写>>,sdsa模型的模块化
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        # <<改写>>,确认sdsa的注意力头和层数
        if args.enc_sdsa_heads is None:
            self.enc_sdsa_heads = [0] * args.encoder_layers
        else:
            self.enc_sdsa_heads = args.enc_sdsa_heads
        assert len(self.enc_sdsa_heads) == args.encoder_layers
        ...
        self.layers.extend(
            [self.build_sdsa_encoder_layer(args, h) if h != 0 \
                 else self.build_encoder_layer(args) for h in self.enc_sdsa_heads]
        )   # <<改写>>,sdsa模型的模块化 # 根据args中编码器层数,由args构建每一层编码器层(与原始论文一致,Multi-headed attn->dropout->添加残差-> layernorm)
    ...
    def build_sdsa_encoder_layer(self, args, h):  # 由args初始化一个TransformerEncoderLayer类
        return TransformerSdsaEncoderLayer(args, h)  # 返回一个与原始论文一致的Transformer编码器层,Multi-headed attn->dropout->添加残差-> layernorm
    ...
    def forward(
        ...
        try:
            ...
            print()
        # encoder layers
        for i, h in enumerate(self.enc_sdsa_heads): # <<改写>>, 由encoder_sdsa_heads遍历每一个编码器层,并添加sdsa和pascal矩阵;
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):  # 若非训练阶段,或者无layer_dropout(0),即均匀随机初始化的dropout值肯定>0:
                ...
                if h != 0:
                    x, attn_weights = self.layers[i](x, encoder_padding_mask, matrix)
                else:
                    x, attn_weights = self.layers[i](x, encoder_padding_mask)  # 返回当前编码器层的输出layer_norm(dropout(max(0, x'W1+b1)W2+b2) + x')
...
@register_model_architecture("transformer_sdsa", "transformer_sdsa")  # <<改写>>,sdsa模型的模块化
def base_architecture(args):
    ...
    # <<改写>>,sdsa模型的模块化
    args.enc_sdsa_heads = getattr(args, "enc_sdsa_heads", None)
    args.prob_q = getattr(args, "prob_q", 0.0)
    args.win_k = getattr(args, "win_k", 0.0)
    args.weight_fn = getattr(args, "weight_fn", "normal")
    args.weight_sigma = getattr(args, "weight_sigma", 2.)
    args.weight_usage = getattr(args, "weight_usage", "mult")
...
@register_model_architecture("transformer_sdsa", "transformer_sdsa_iwslt_de_en")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_iwslt_de_en(args):  # <<改写>>,sdsa模型的模块化
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_sdsa", "transformer_sdsa_wmt_en_de")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_wmt_en_de(args):  # 通过-arch选项选择合适的transformer参数  # <<改写>>,sdsa模型的模块化
    base_architecture(args)  # 针对args提供进来的参数,已存在的不变,未设置的则按base_architecture设置


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_sdsa", "transformer_sdsa_vaswani_wmt_en_de_big")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_vaswani_wmt_en_de_big(args):  # <<改写>>,sdsa模型的模块化
...
@register_model_architecture("transformer_sdsa", "transformer_sdsa_vaswani_wmt_en_fr_big")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_vaswani_wmt_en_fr_big(args):  # <<改写>>,sdsa模型的模块化
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_sdsa_vaswani_wmt_en_de_big(args)  # <<改写>>,sdsa模型的模块化


@register_model_architecture("transformer_sdsa", "transformer_sdsa_wmt_en_de_big")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_wmt_en_de_big(args):  # <<改写>>,sdsa模型的模块化
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_sdsa_vaswani_wmt_en_de_big(args)  # <<改写>>,sdsa模型的模块化


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_sdsa", "transformer_sdsa_wmt_en_de_big_t2t")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_wmt_en_de_big_t2t(args):  # <<改写>>,sdsa模型的模块化
