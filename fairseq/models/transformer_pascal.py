...
from fairseq.models import (
    FairseqEncoder,   # pascal模型的编码器的父类
    FairseqEncoderDecoderModel,
    FairseqIncrementalDecoder,
    register_model,
    register_model_architecture,
)
from fairseq.models.transformer import TransformerDecoder  # pascal模型的解码器
...
from fairseq.modules import (
    ...
    TransformerEncoderLayer,   # pascal模型的san编码层
    TransformerPascalEncoderLayer,   # <<改写>>,pascal模型的sdsa编码层
)
...
@register_model("transformer_pascal")  # <<改写>>,pascal模型的模块化
class TransformerPascalModel(FairseqEncoderDecoderModel):  # <<改写>>,pascal模型的模块化
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--enc-pascal-heads', type=int, nargs='+',
        ...
        # <<改写>>,设置Pascal矩阵的的编码端的标准差,默认为1.
        parser.add_argument('--weight-sigma', type=float, metavar='D',
                            help="parameter of Pascal weight function (std or window size)")
        # <<改写>>,设置Pascal的parent-ignoring概率,默认为0.0
        parser.add_argument('--parent-ignoring', type=float, metavar='D',
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # 通过args,源字典以及源端词嵌入建立编码器,初始化TransformerEncoder类
        return TransformerPascalEncoder(args, src_dict, embed_tokens)  # <<改写>>,Pascal模型的模块化 # 返回一个与原始论文一致的Transformer Encoder,每一层TransformerEncoderLayer(Multi-headed attn->dropout->添加残差-> layernorm)
...
class TransformerPascalEncoder(FairseqEncoder):  # <<改写>>,pascal模型的模块化
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        # <<改写>>,确认pascal的注意力头和层数
        if args.enc_pascal_heads is None:
            self.enc_pascal_heads = [0] * args.encoder_layers
        else:
            self.enc_pascal_heads = args.enc_pascal_heads
        assert len(self.enc_pascal_heads) == args.encoder_layers
        ...
        self.layers.extend(
            [self.build_pascal_encoder_layer(args, h) if h != 0 \
                 else self.build_encoder_layer(args) for h in self.enc_pascal_heads]
        )   # <<改写>>,pascal模型的模块化 # 根据args中编码器层数,由args构建每一层编码器层(与原始论文一致,Multi-headed attn->dropout->添加残差-> layernorm)
    ...
    def build_pascal_encoder_layer(self, args, h):  # 由args初始化一个TransformerEncoderLayer类
        return TransformerPascalEncoderLayer(args, h)  # 返回一个与原始论文一致的Transformer编码器层,Multi-headed attn->dropout->添加残差-> layernorm
    ...
    def forward(
        ...
        # encoder layers
        for i, h in enumerate(self.enc_pascal_heads): # <<改写>>, 由enc_pascal_heads遍历每一个编码器层,并添加sdsa和pascal矩阵;
...
@register_model_architecture("transformer_pascal", "transformer_pascal")  # <<改写>>,pascal模型的模块化
def base_architecture(args):
    ...
    # <<改写>>,pascal模型的模块化
    args.enc_pascal_heads = getattr(args, "enc_pascal_heads", None)
    args.parent_ignoring = getattr(args, "parent_ignoring", 0.0)
    args.weight_fn = getattr(args, "weight_fn", "normal")
    args.weight_sigma = getattr(args, "weight_sigma", 1.)
...
@register_model_architecture("transformer_pascal", "transformer_pascal_iwslt_de_en")  # <<改写>>,pascal模型的模块化
def transformer_pascal_iwslt_de_en(args):  # <<改写>>,pascal模型的模块化
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_pascal", "transformer_pascal_wmt_en_de")  # <<改写>>,pascal模型的模块化
def transformer_pascal_wmt_en_de(args):  # 通过-arch选项选择合适的transformer参数  # <<改写>>,sdsa模型的模块化
    base_architecture(args)  # 针对args提供进来的参数,已存在的不变,未设置的则按base_architecture设置


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_pascal", "transformer_pascal_vaswani_wmt_en_de_big")  # <<改写>>,pascal模型的模块化
def transformer_pascal_vaswani_wmt_en_de_big(args):  # <<改写>>,pascal模型的模块化
...
@register_model_architecture("transformer_pascal", "transformer_pascal_vaswani_wmt_en_fr_big")  # <<改写>>,pascal模型的模块化
def transformer_pascal_vaswani_wmt_en_fr_big(args):  # <<改写>>,pascal模型的模块化
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_pascal_vaswani_wmt_en_de_big(args)  # <<改写>>,pascal模型的模块化


@register_model_architecture("transformer_pascal", "transformer_pascal_wmt_en_de_big")  # <<改写>>,pascal模型的模块化
def transformer_pascal_wmt_en_de_big(args):  # <<改写>>,pascal模型的模块化
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_pascal_vaswani_wmt_en_de_big(args)  # <<改写>>,pascal模型的模块化


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_pascal", "transformer_pascal_wmt_en_de_big_t2t")  # <<改写>>,pascal模型的模块化
def transformer_pascal_wmt_en_de_big_t2t(args):  # <<改写>>,pascal模型的模块化
