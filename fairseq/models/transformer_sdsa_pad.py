...
from fairseq.modules import (
    ...
    TransformerSdsaPadEncoderLayer,   # <<改写>>,sdsa模型的sdsa编码层
)
...
@register_model("transformer_sdsa_pad")  # <<改写>>,sdsa模型的模块化
class TransformerSdsaPadModel(FairseqEncoderDecoderModel):  # <<改写>>,sdsa模型的模块化
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # 通过args,源字典以及源端词嵌入建立编码器,初始化TransformerEncoder类
        return TransformerSdsaPadEncoder(args, src_dict, embed_tokens)  # <<改写>>,Sdsa模型的模块化 # 返回一个与原始论文一致的Transformer Encoder,每一层TransformerEncoderLayer(Multi-headed attn->dropout->添加残差-> layernorm)
...
class TransformerSdsaPadEncoder(FairseqEncoder):  # <<改写>>,sdsa模型的模块化
    """
    ...
    def build_sdsa_encoder_layer(self, args, h):  # 由args初始化一个TransformerEncoderLayer类
        return TransformerSdsaPadEncoderLayer(args, h)  # 返回一个与原始论文一致的Transformer编码器层,Multi-headed attn->dropout->添加残差-> layernorm
    ...
    def forward(
        ...
        for i, h in enumerate(self.enc_sdsa_heads): # <<改写>>, 由encoder_sdsa_heads遍历每一个编码器层,并添加sdsa和pascal矩阵;
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):  # 若非训练阶段,或者无layer_dropout(0),即均匀随机初始化的dropout值肯定>0:
                ...
                if h != 0:
                    x = self.layers[i](x, encoder_padding_mask, matrix)
                else:
...
@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad")  # <<改写>>,sdsa模型的模块化
...
@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad_iwslt_de_en")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_pad_iwslt_de_en(args):  # <<改写>>,sdsa模型的模块化
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad_wmt_en_de")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_pad_wmt_en_de(args):  # 通过-arch选项选择合适的transformer参数  # <<改写>>,sdsa模型的模块化
    base_architecture(args)  # 针对args提供进来的参数,已存在的不变,未设置的则按base_architecture设置


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad_vaswani_wmt_en_de_big")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_pad_vaswani_wmt_en_de_big(args):  # <<改写>>,sdsa模型的模块化
...
@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad_vaswani_wmt_en_fr_big")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_pad_vaswani_wmt_en_fr_big(args):  # <<改写>>,sdsa模型的模块化
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_sdsa_pad_vaswani_wmt_en_de_big(args)  # <<改写>>,sdsa模型的模块化


@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad_wmt_en_de_big")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_pad_wmt_en_de_big(args):  # <<改写>>,sdsa模型的模块化
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_sdsa_pad_vaswani_wmt_en_de_big(args)  # <<改写>>,sdsa模型的模块化


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_sdsa_pad", "transformer_sdsa_pad_wmt_en_de_big_t2t")  # <<改写>>,sdsa模型的模块化
def transformer_sdsa_pad_wmt_en_de_big_t2t(args):  # <<改写>>,sdsa模型的模块化
