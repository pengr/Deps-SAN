...
from fairseq.models.transformer import TransformerDecoder  # DepIntvlRpe模型的解码器
...
from fairseq.modules import (
    ...
    TransformerDepIntvlRpeEncoderLayer,   # <<改写>>,DepIntvlRpe模型的编码层
)
...
@register_model("transformer_dep_intvl_rpe") # <<改写>>,DepIntvlRpe模型的模块化
class TransformerDepIntvlRpeModel(FairseqEncoderDecoderModel): # <<改写>>,DepIntvlRpe模型的模块化
    """
    ...
    def add_args(parser):
        ...
        # <<改写>>,DepIntvlRpe模型的模块化
        parser.add_argument('--max-dependency-interval', type=int, metavar='N',
                            help="Maximum dependency interval between inputs in dependency-interval"
                                 "relative positions representations.")
        # <<改写>>,DepIntvlRpe模型的模块化
        parser.add_argument('--no-key-dep-intvl-rpe', default=False, action='store_true',
                            help='if set, disables dependency-interval relative positional embeddings for key (Rk)')
        # <<改写>>,DepIntvlRpe模型的模块化
        parser.add_argument('--no-value-dep-intvl-rpe', default=False, action='store_true',
                            help='if set, disables dependency-interval relative positional embeddings for value (Rv)')
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # 通过args,源字典以及源端词嵌入建立编码器,初始化TransformerEncoder类
        return TransformerDepIntvlRpeEncoder(args, src_dict, embed_tokens)  # <<改写>>,DepIntvlRpe模型的模块化 # 返回一个与原始论文一致的Transformer Encoder,每一层TransformerEncoderLayer(Multi-headed attn->dropout->添加残差-> layernorm)
...
class TransformerDepIntvlRpeEncoder(FairseqEncoder): # <<改写>>,DepIntvlRpe模型的模块化
    """
    ...
    def build_encoder_layer(self, args):   # <<改写>>,DepIntvlRpe模型的模块化 # 由args初始化一个TransformerEncoderLayer类
        return TransformerDepIntvlRpeEncoderLayer(args)  # 返回一个与原始论文一致的Transformer编码器层,Multi-headed attn->dropout->添加残差-> layernorm
    ...
    def forward(
        ...
        for layer in self.layers:  # 遍历每一个编码器层
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):  # 若非训练阶段,或者无layer_dropout(0),即均匀随机初始化的dropout值肯定>0:
                ...
                x = layer(x, encoder_padding_mask, matrix)   # <<改写>>, 添加sdsa和pascal矩阵; # 返回当前编码器层的输出layer_norm(dropout(max(0, x'W1+b1)W2+b2) + x')
...
@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe") # <<改写>>,DepIntvlRpe模型的模块化
def base_architecture(args):
    ...
    args.max_dependency_interval = getattr(args, "max_dependency_interval", 0)   # <<改写>>,DepIntvlRpe模型的模块化
    args.no_key_dep_intvl_rpe = getattr(args, "no_key_dep_intvl_rpe", False)     # <<改写>>,DepIntvlRpe模型的模块化
    args.no_value_dep_intvl_rpe = getattr(args, "no_value_dep_intvl_rpe", False)  # <<改写>>,DepIntvlRpe模型的模块化
...
@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe_iwslt_de_en") # <<改写>>,DepIntvlRpe模型的模块化
def transformer_dep_intvl_rpe_iwslt_de_en(args): # <<改写>>,DepIntvlRpe模型的模块化
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe_wmt_en_de") # <<改写>>,DepIntvlRpe模型的模块化
def transformer_dep_intvl_rpe_wmt_en_de(args):   # <<改写>>,DepIntvlRpe模型的模块化　# 通过-arch选项选择合适的transformer参数
    base_architecture(args)  # 针对args提供进来的参数,已存在的不变,未设置的则按base_architecture设置


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe_vaswani_wmt_en_de_big") # <<改写>>,DepIntvlRpe模型的模块化
def transformer_dep_intvl_rpe_vaswani_wmt_en_de_big(args): # <<改写>>,DepIntvlRpe模型的模块化
...
@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe_vaswani_wmt_en_fr_big") # <<改写>>,DepIntvlRpe模型的模块化
def transformer_dep_intvl_rpe_vaswani_wmt_en_fr_big(args): # <<改写>>,DepIntvlRpe模型的模块化
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_dep_intvl_rpe_vaswani_wmt_en_de_big(args) # <<改写>>,DepIntvlRpe模型的模块化


@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe_wmt_en_de_big") # <<改写>>,DepIntvlRpe模型的模块化
def transformer_dep_intvl_rpe_wmt_en_de_big(args): # <<改写>>,DepIntvlRpe模型的模块化
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_dep_intvl_rpe_vaswani_wmt_en_de_big(args) # <<改写>>,DepIntvlRpe模型的模块化


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_dep_intvl_rpe", "transformer_dep_intvl_rpe_wmt_en_de_big_t2t") # <<改写>>,DepIntvlRpe模型的模块化
def transformer_dep_intvl_rpe_wmt_en_de_big_t2t(args): # <<改写>>,DepIntvlRpe模型的模块化
