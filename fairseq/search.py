...
class Search(nn.Module):
    def __init__(self, tgt_dict):
        super().__init__()
        self.pad = tgt_dict.pad()  # 获取目标字典的pad/unk/eos符号对应idx
        self.unk = tgt_dict.unk()
        self.eos = tgt_dict.eos()
        self.vocab_size = len(tgt_dict) # 获取目标字典的词汇表大小
        self.src_lengths = torch.tensor(-1) # 初始化一个源句子长度
    ...
    @torch.jit.export
    def set_src_lengths(self, src_lengths): # 设置当前遍历的batch中全部源句子的长度的Tensor(不带eos/pad);
        self.src_lengths = src_lengths


class BeamSearch(Search):
    def __init__(self, tgt_dict):
        super().__init__(tgt_dict) # 调用父类Search加载目标字典的pad/unk/eos符号对应idx,词汇表大小,初始化一个源句子长度

    @torch.jit.export
    def step(self, step: int, lprobs, scores: Optional[Tensor]):
        bsz, beam_size, vocab_size = lprobs.size() # 从当前解码时步的模型输出中获取归一化的概率(或对数概率)lprobs,得到bsz,beam_size,tgt_vocab_size

        if step == 0:
            # at the first step all hypotheses are equally likely, so use
            # only the first beam 第一个解码时间步,所有假设的可能性均等,故仅使用第一个波束
            lprobs = lprobs[:, ::beam_size, :].contiguous()  # 从lprobs的第2个维度中(beam size)取出第1列(即仅使用第一个波束),形状为bsz,1,tgt_vocab_size
        else:
            # make probs contain cumulative scores for each hypothesis # 使概率包含每个假设的累积分数
            assert scores is not None
            lprobs.add_(scores[:, :, step - 1].unsqueeze(-1)) # 当前解码时步的模型输出中获取归一化的概率(或对数概率)lprobs+前一解码时间步的scores扩维为[bsz,beam_size,1]

        top_prediction = torch.topk(
            lprobs.view(bsz, -1),  # 第一个解码时间步为bsz,tgt_vocab_size;后续解码时间步为bsz,beam*tgt_vocab_size
            k=min(
                # Take the best 2 x beam_size predictions. We'll choose the first
                # beam_size of these which don't predict eos to continue with. # 进行最佳的2xbeam_size预测.我们将选择其中的第一个beam_size,它们不会预测eos会继续下去
                beam_size * 2,
                lprobs.view(bsz, -1).size(1) - 1,  # -1 so we never select pad # -1表示我们绝不预测出pad符号,此值最少为tgt_vocab_size-1
            ),  # k值默认为2*beam_size
        ) # 从包含每个假设的累积分数(或仅使用第一个波束)的当前解码时步的模型输出中获取归一化的概率(或对数概率)lprobs,选取最后一个dim维度上最大的2xbeam_size个值及其对应的indices
        scores_buf = top_prediction[0] # 从包含每个假设的累积分数(或仅使用第一个波束)的当前解码时步的模型输出中获取归一化的概率(或对数概率)lprobs,选取最后一个dim维度上最大的2xbeam_size个值
        indices_buf = top_prediction[1]# 从包含每个假设的累积分数(或仅使用第一个波束)的当前解码时步的模型输出中获取归一化的概率(或对数概率)lprobs,选取最后一个dim维度上最大的2xbeam_size个值对应的indices
        beams_buf = torch.div(indices_buf, vocab_size) # 将当前解码时间步最大2xbeam_size的归一化的概率值对应的indices/目标词汇表大小,商代表其所属于第几个beam上
        indices_buf.fmod_(vocab_size)  # 将当前解码时间步最大2xbeam_size的归一化的概率值对应的indices/目标词汇表大小,余数代表其在词汇表的索引
