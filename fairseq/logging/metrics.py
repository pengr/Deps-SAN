...
def aggregate(name: Optional[str] = None, new_root: bool = False):
    """Context manager to aggregate metrics under a given name. 在给定名称下整合指标的上下文管理器
...
def log_scalar(
    ...
    """Log a scalar value. 记录一个标量值

    Args:
        key (str): name of the field to log　 # 要记录的字段名称
        value (float): value to log          # 要记录的值
        weight (float): weight that this value contributes to the average.
            A weight of 0 will always log the latest value.　# 该值有助于平均值的权重,权重0表示将始终记录最新值
        priority (int): smaller values are logged earlier in the output
        round (Optional[int]): number of digits to round to when displaying # 较小的值记录在输出中的较早位置,显示时四舍五入到的位数
    """
    for agg in get_active_aggregators():  # 将key字段名称记录到agg(一个MetersDict)中,若没有则由add_meter先添加
        if key not in agg:
            agg.add_meter(key, AverageMeter(round=round), priority)
        agg[key].update(value, weight)  # 在agg中更新key:(value,weight)键值对,其中只有value在变


def log_derived(key: str, fn: Callable[[MetersDict], float], priority: int = 20):
    """Log a scalar value derived from other meters. 记录从其他meters得出的标量值
...
def log_start_time(key: str, priority: int = 40, round: Optional[int] = None):
    """Log the duration of some event in seconds. 记录某些事件的持续时间（以秒为单位）
...
def get_smoothed_values(name: str) -> Dict[str, float]:
    ...
    """
    return _aggregators[name].get_smoothed_values()  # 获取在给定*name*下所汇聚的平滑值
