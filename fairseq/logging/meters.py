...
def safe_round(number, ndigits):  # 将safe_round函数运行ndigits,确保number值被正确处理
...
class TimeMeter(Meter):
    """Computes the average occurrence of some event per second 计算每秒某事件的平均发生率"""
...
class StopwatchMeter(Meter):
    """Computes the sum/avg duration of some event in seconds 计算某事件的总和/平均持续时间(s)"""
    ...
    def start(self):  # 记录代码所持续的时间(两次调用之间的时间)
...
class MetersDict(OrderedDict):
    ...
    def get_smoothed_value(self, key: str) -> float:
        ...
        else: # 默认情况,返回当前key的smoothed_value
    ...
    def get_smoothed_values(self) -> Dict[str, float]:
        ...
        ])  # 返回MetersDict中非"_"开头的key,以及其smoothed_value所组成的有序字典
